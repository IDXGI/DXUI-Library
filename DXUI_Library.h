...
protected:
	...
	inline bool _DrawDispose(ID2D1RenderTarget*& DstTarget, const D2D1_COLOR_F& BackColor, D2D1_RECT_F*& UpdateClip, ID2D1SolidColorBrush*& brush)
	{
		if (!DstTarget)
		{
			this->_SetNeedUpdate();
	...
	inline D2D1_RECT_F _GetUpdateRect(D2D1_RECT_F* UpdateClip)//为主窗口显示刷新区域所用
	{
		D2D1_RECT_F r;
		___RcChangedLock.Lock();
		if (RectChanged)
		...
		if (EnableRenderClip)
		{
			...
			if (ofs == 0.0f)
				...
				r = CalcRectOverlap({ RenderClip.left, RenderClip.top - ofs, RenderClip.right, RenderClip.bottom - ofs }, r);
		}
	...
	void _UpdateBackImg(ID2D1RenderTarget* Target, D2D1_RECT_F* UpdateClip = nullptr)
	{
		...
		if (!BackImg)
		{
			...
			Target->CreateBitmap({ (UINT)ceil(UpdateRC.right * (float)dpiX / 96.0f - UpdateRC.left * (float)dpiX / 96.0f), (UINT)ceil(UpdateRC.bottom * (float)dpiX / 96.0f - UpdateRC.top * (float)dpiY / 96.0f) }, prop, &BackImg);
		}
		if (!BackImg)
		{
		...
		if (UpdateClip)
		{
			...
			src.left = (UINT)floor(r.left * (float)dpiX / 96.0f);
			src.top = (UINT)floor(r.top * (float)dpiY / 96.0f);
			src.right = (UINT)ceil(r.right * (float)dpiX / 96.0f);
			src.bottom = (UINT)ceil(r.bottom * (float)dpiY / 96.0f);
			dst = { (UINT)ceil((r.left - updrc.left) * (float)dpiX / 96.0f), (UINT)ceil((r.top - updrc.top) * (float)dpiY / 96.0f) };
		}
		else
		{
	...
	inline void _DrawBackImg(ID2D1RenderTarget* Target)
	{
		...
		if (BackImg)
		{
			...
			upd.top = floor((UpdateRC.top - ofs) * (float)dpiY / 96.0f) * 96.0f / (float)dpiY + ofs;//消除误差
	...
	bool _Draw(ID2D1RenderTarget* DstTarget, const D2D1_COLOR_F& BackColor, D2D1_RECT_F* UpdateClip)
	{
		...
		if (TextLayout && f_TextColor.a != 0.0f)
		{
			...
			if (met.widthIncludingTrailingWhitespace >= met.layoutWidth || met.height >= met.layoutHeight)
				Clip.Push(DstTarget, DstRC.rect);
...
public:
	...
	void SetFullHeight(float fullheight)
	{
		...
		if (FullHeight - Scr < DstRC.rect.bottom - DstRC.rect.top)//太靠上时的处理
		{
	...
	void SetScrollOffset(float Offset, UINT time = 0, DBezier* Nonlinear = nullptr)
	{
		...
		{
			if (time)
				A_Scr.AddAni(src, Scr, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear, 1);
			else
				ScrOffset = floor(Scr * (float)dpiY / 96.0f) * 96.0f / (float)dpiY;
