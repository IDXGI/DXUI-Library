...
*******************************************************************/
#define DXUI_VERSION 220206//DXUI版本
...
public:
	...
	float FrameCalc(UINT* aid) noexcept
	{
		...
		if (!nNowFrame)
		{
			if (aid && *aid == 0)//这里只有最后一帧赋值，其他情况不赋值
				*aid = AID;
			cache = End;
		}
		else
...
protected:
	...
	inline void _Base_DisposeAni()
	{
		if (A_RC_width || A_RC_height)
		{
			...
			if (resp)
				ResponseCur();
			EndAniSend(A_RC_width || A_RC_height ? 0 : AID);
		}
		if (A_RC_x || A_RC_y)
		{
			UINT AID = 0;
			...
			if (resp)
				ResponseCur();
			EndAniSend(A_RC_x || A_RC_y ? 0 : AID);
		}
...
protected:
	...
	void _DisposeAnimation()//由主窗口每帧后主动调用此函数，来实现每次的动画
	{
		...
		if (A_Radius_X || A_Radius_Y)
		{
			...
			if (resp)
				ResponseCur();
			EndAniSend(A_Radius_X || A_Radius_Y ? 0 : AID);
		}
		if (A_Opa)
		{
			UINT AID = 0;
...
protected:
	...
	void _DisposeAnimation()//由主窗口每帧后主动调用此函数，来实现每次的动画
	{
		...
		if (A_RRad_X || A_RRad_Y)
		{
			...
			this->_SetNeedUpdate();
			EndAniSend(A_RRad_X || A_RRad_Y ? 0 : AID);
		}
		if (A_RRad_Opa)
		{
			UINT AID = 0;
...
protected:
	...
	void _DisposeAnimation()//由主窗口每帧后主动调用此函数，来实现每次的动画
	{
		...
		if (A_Sta_X || A_Sta_Y)
		{
			...
			this->_SetNeedUpdate();
			EndAniSend(A_Sta_X || A_Sta_Y ? 0 : AID);
		}
		if (A_End_X || A_End_Y)
		{
			UINT AID = 0;
			...
			this->_SetNeedUpdate();
			EndAniSend(A_End_X || A_End_Y ? 0 : AID);
		}
		if (A_Lin_Opa)
		{
			UINT AID = 0;
...
protected:
	...
	void _DisposeAnimation()//由主窗口每帧后主动调用此函数，来实现每次的动画
	{
		...
		if (A_Cen_X || A_Cen_Y)
		{
			...
			this->_SetNeedUpdate();
			EndAniSend(A_Cen_X || A_Cen_Y ? 0 : AID);
		}
		if (A_RRad_X || A_RRad_Y)
		{
			UINT AID = 0;
