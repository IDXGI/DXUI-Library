...
protected:
	...
	bool _EndDraw_CallChildToUpdateBackImg(ID2D1RenderTarget* CacheTarget, std::map<UINT, __updItem>& UpdCon)
	{
		bool res = false;
		___ChildLock.Lock();
		if (!Child.empty())
			...
			if (_IsContentChanging())//将Child全部设为要更新
			{
				...
				for (UINT i = 0; i < Child.size(); ++i, ++child)
				{
					...
					if (EnableClip)
					{
						...
						{
							if (UpdCon.find(child->first) == UpdCon.end())
							{
								child->second->___UpdateClipLock.Lock();
								UpdCon.insert(std::pair<UINT, __updItem>(child->first, { child->second, child->second->EnableClip, child->second->UpdateClip }));
...
public:
	...
	void SetPosition(float x, float y, UINT time = 0, DBezier* Nonlinear = nullptr, UINT AID = 0)
	{
		...
		if (Parent)
		{
			...
			Parent->_SetNeedUpdate(CalcRectUnion(UpdateRC, OldUpdateRC));
		}
		else
			this->_SetNeedUpdate();
		return;
	...
	inline bool _RenderFrame(bool Lock = true)
	{
		...
		if (!Controls.empty())
		{
				...
				if (con->second)
				{
					...
					if (con->second->_GetNeedUpdate())
					{
						...
						if (!con->second->Parent)
						{
							...
							if (insIt.first->second.EnableUpdClip)
								con->second->_NoParentClearBackGru(RenderTarget, BackColor, &insIt.first->second.UpdClip);
							else
								con->second->_NoParentClearBackGru(RenderTarget, BackColor, nullptr);
						}
					}//NeedUpdate End
				}
			}//for End
			...
			while (UpdConIt != UpdCon.end())
			{
				...
				if (UpdConIt->second.EnableUpdClip)
					UpdConIt->second.con->_Draw(RenderTarget, BackColor, &UpdConIt->second.UpdClip);//将当前控件画到缓存渲染目标上
				else
					UpdConIt->second.con->_Draw(RenderTarget, BackColor, nullptr);//将当前控件画到缓存渲染目标上

				if (UpdConIt->second.con->_EndDraw_CallChildToUpdateBackImg(RenderTarget, UpdCon))
				...
				if (Dbg_ShowUpdate)
				{
					...
					if (b)
					{
						RenderTarget->DrawRectangle(UpdConIt->second.con->_GetUpdateRect(), b, 1.0f);
						b->Release();
					}
				}
	...
	inline bool _RenderFrame(bool Lock = true)
	{
		...
		if (!Controls.empty())
		{
				...
				if (con->second)
				{
					...
					if (con->second->_GetNeedUpdate())
					{
						...
						if (!con->second->Parent)
						{
							...
							if (insIt.first->second.EnableUpdClip)
								con->second->_NoParentClearBackGru(DCTarget, BackColor, &insIt.first->second.UpdClip);
							else
								con->second->_NoParentClearBackGru(DCTarget, BackColor, nullptr);
						}
					}//NeedUpdate End
				}
			}//for End
			...
			while (UpdConIt != UpdCon.end())
			{
				...
				if (UpdConIt->second.EnableUpdClip)
					UpdConIt->second.con->_Draw(DCTarget, BackColor, &UpdConIt->second.UpdClip);//将当前控件画到缓存渲染目标上
				else
					UpdConIt->second.con->_Draw(DCTarget, BackColor, nullptr);//将当前控件画到缓存渲染目标上

				if (UpdConIt->second.con->_EndDraw_CallChildToUpdateBackImg(DCTarget, UpdCon))
					UpdConIt = UpdCon.find(nowID);
				...
				if (Dbg_ShowUpdate)
				{
					...
					if (b)
					{
						DCTarget->DrawRectangle(UpdConIt->second.con->_GetUpdateRect(), b, 1.0f);
						b->Release();
					}
				}
