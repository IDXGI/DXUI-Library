...
private:
	...
	bool _Draw(ID2D1BitmapRenderTarget* DstTarget, D2D1_COLOR_F BackColor, D2D1_RECT_F MainWndRC)
	{
		...
		if (NowFrameCount)//如果还有动画没有播放完
		{
			...
			//静态——文字颜色处理
			NowType.r = (float)DstType->TextColor.r;
			NowType.g = (float)DstType->TextColor.g;
			NowType.b = (float)DstType->TextColor.b;
			NowType.a = DstType->TextColor.a;
...
private:
	...
	inline bool MyLoadBmp(ID2D1RenderTarget* Target, BYTE* data, ULONG Size, ID2D1Bitmap** bmp, D2D1_RECT_F* bmprc)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(WicFactory->CreateBitmapFromSource(pFormat, WICBitmapNoCache, &WicBmp)))
			{
				if (SUCCEEDED(WicBmp->Lock(&rect, WICBitmapLockRead, &lock)) && lock)
				{
					lock->GetStride(&pitch);
					lock->GetDataPointer(&BufferSize, &cv);

					if (CacheBmp && cv)
						CacheBmp->CopyFromMemory(&d2drc, cv, pitch);

					RELEASE(lock);

					//将CacheBmp画到dstBitmap上，用于调整到符合屏幕的大小，避免每次绘制都拉伸图片

					ID2D1BitmapRenderTarget* dstRenderTarget;
					D2D1_SIZE_F DstSize;
					...
					Target->CreateCompatibleRenderTarget(DstSize, &dstRenderTarget);
					...
					if (CacheBmp)
					{
						...
						dstrc.left = 0;
						dstrc.top = 0;
						dstrc.right = DstSize.width;
						dstrc.bottom = DstSize.height;

						D2D1_RECT_F src;
						src.left = 0;
						src.top = 0;
						src.right = (float)wid;
						src.bottom = (float)hei;

						dstRenderTarget->BeginDraw();

						dstRenderTarget->DrawBitmap(CacheBmp, dstrc, 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_LINEAR, src);

						dstRenderTarget->EndDraw();

						RELEASE(*bmp);
						dstRenderTarget->GetBitmap(bmp);

						RELEASE(CacheBmp);
					}
					RELEASE(dstRenderTarget);
...
public:
	...
	inline void DeleteControl(UINT ControlID)
	{
		if (!Controls.empty())
		{
			std::map<UINT, DControl*>::iterator con = Controls.find(ControlID);
			if (con != Controls.end())
			{
				con->second->_SeparateFromWindow();

				DControl* parent = con->second->_GetParentCon();
				if (parent)
					parent->_GetParentCon()->_DeleteChildCon(con->second);

				std::map<UINT, DControl*> child = con->second->_GetChildCon();
				if (!child.empty())
				{
					std::map<UINT, DControl*>::iterator chi = child.begin();
					for (UINT i = 0; i < child.size(); i++, chi++)
					{
						chi->second->_SetParentCon(parent);//将欲删除的控件的child的父控件设为欲删除控件的父控件
					}
				}

				con->second->_ClearChildCon();
	...
	DReturnMsg DXUIMessageProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		if (hWnd == this->hWnd && !Controls.empty())
		{
			...
			case WM_NCMOUSEMOVE:
			{
				...
				___ConLock.BeginRead();
				std::map<UINT, DControl*>::iterator con = Main_HitTest(p.x, p.y);//命中测试
				if (con != Controls.end())
				{
					RestoreLastActiveControl(con->second->GetID(), message);
					if (con->second->_MouseMoveMsgProc(p.x, p.y, &msg.Msg))
					{
						msg.ControlID = con->second->GetID();
			...
			case WM_NCHITTEST:
			{
				...
				___ConLock.BeginRead();
				std::map<UINT, DControl*>::iterator con = Main_HitTest(p.x, p.y);//命中测试
				if (con != Controls.end() && con->second->_GetMoveWindow())
				{
					msg.ReturnValue = HTCAPTION;
				}
				___ConLock.EndRead();
			...
			case WM_NCLBUTTONDOWN:
			{
				...
				if (con != Controls.end())
				{
					...
					con->second->_LMouseDownMsgProc(p.x, p.y, &msg.Msg);
			...
			case WM_NCLBUTTONUP:
			{
				...
				if (con != Controls.end())
				{
					...
					con->second->_LMouseUpMsgProc(p.x, p.y, &msg.Msg);
