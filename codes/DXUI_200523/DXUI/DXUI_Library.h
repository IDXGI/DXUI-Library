...
#define TextBox_SmoothScrollTime 160//文本框的平滑滚动时间(ms)
#define TextBox_CaretMoveTime 130//文本框的插入符平滑移动时间(ms)
...
protected:
	...
	void _PreDisposeRC()
	{
		...
		if (StrokeWidth != 0.0f)
		{
			...
			UpdateRC.right = (ceil(DstRC.rect.right * (float)dpiX / 96.0f) + 1.0f) * 96.0f / (float)dpiX;
			UpdateRC.bottom = (ceil(DstRC.rect.bottom * (float)dpiY / 96.0f) + 1.0f) * 96.0f / (float)dpiY;
		}
		return;
	}
	...
	void _UpdateBackImg(ID2D1RenderTarget* Target, D2D1_RECT_F* UpdateClip = nullptr)
	{
		...
		if (UpdateClip)
		{
			...
			dst = { src.left - (UINT)(UpdateRC.left * dpiX / 96.0f), src.top - (UINT)(UpdateRC.top * dpiY / 96.0f) };
		}
		else
		{
	...
	bool _EndDraw_CallChildToUpdateBackImg(ID2D1RenderTarget* CacheTarget, std::map<UINT, __updItem>& UpdCon, D2D1_RECT_F* mUpdateClip)
	{
		...
		if (!Child.empty())
		{
			...
			for (UINT i = 0; i < Child.size(); ++i, ++child)
			{
				...
				if (mUpdateClip)
				{
					if (IsRectWorkable(CalcRectOverlap(child->second->_GetUpdateRC(), *mUpdateClip)))
					{
						...
						child->second->_UpdateBackImg(CacheTarget, mUpdateClip);

						if (upd == UpdCon.end())
						{
							UpdCon.insert(std::pair<UINT, __updItem>(child->first, { child->second, true, child->second->EnableClip ? CalcRectUnion(*mUpdateClip, child->second->UpdateClip) : *mUpdateClip }));
							...
							upd->second.UpdClip = CalcRectUnion(*mUpdateClip, upd->second.UpdClip);
						}
...
private:
	...
	bool _Draw(ID2D1RenderTarget* DstTarget, const D2D1_COLOR_F& BackColor, D2D1_RECT_F* UpdateClip)
	{
		...
		if (TextLayout && f_TextColor.a != 0.0f)
		{
			...
			DstTarget->DrawTextLayout({ (float)DstRC.rect.left, (float)DstRC.rect.top }, TextLayout, brush, D2D1_DRAW_TEXT_OPTIONS_ENABLE_COLOR_FONT);
		}
		___LayoutLock.Unlock();
...
private:
	...
	bool _Draw(ID2D1RenderTarget* DstTarget, const D2D1_COLOR_F& BackColor, D2D1_RECT_F* UpdateClip)
	{
		...
		{
			brush->SetColor(f_TextColor);//设置为文字颜色
			DstTarget->DrawTextLayout({ floor((DstRC.rect.left * (float)dpiX / 96.0f)) * 96.0f / (float)dpiX, floor((DstRC.rect.top * (float)dpiY / 96.0f)) * 96.0f / (float)dpiY }, TextLayout, brush, D2D1_DRAW_TEXT_OPTIONS_ENABLE_COLOR_FONT);
		}
		___LayoutLock.Unlock();
...
private:
	...
	bool _Draw(ID2D1RenderTarget* DstTarget, const D2D1_COLOR_F& BackColor, D2D1_RECT_F* UpdateClip)
	{
		...
		{
			brush->SetColor(f_TextColor);//设置为文字颜色
			DstTarget->DrawTextLayout({ floor(((TextOutputRC.left - Scr_Offset.x) * (float)dpiX / 96.0f)) * 96.0f / (float)dpiX, floor(((TextOutputRC.top - Scr_Offset.y) * (float)dpiY / 96.0f)) * 96.0f / (float)dpiY }, TextLayout, brush, D2D1_DRAW_TEXT_OPTIONS_ENABLE_COLOR_FONT);
		}
		___LayoutLock.Unlock();
...
public:
	...
	bool Init(POINT position, SIZE size, DCOLOR color, float radiusX = 0.0f, float radiusY = 0.0f, bool CanMoveWindow = false)
	{
		if (Inited)
			return false;
...
protected:
	...
	bool _IsContentChanging()
	{
		if (A_Col || A_RC_x || A_RC_y || A_RC_width || A_RC_height || A_Radius_X || A_Radius_Y || A_Opa)
			return true;
		else
			return false;
	}
...
public:
	...
	bool Init(POINT position, SIZE size, DCOLOR BackColor, DColorBox_Point* Points, UINT PointCount, POINT StartPos, POINT EndPos, float radiusX = 0.0f, float radiusY = 0.0f, bool CanMoveWindow = false)
	{
		if (Inited)
			return false;
...
private:
	...
	bool _IsContentChanging()
	{
		if (A_Col || A_RC_x || A_RC_y || A_RC_width || A_RC_height || A_Radius_X || A_Radius_Y || A_Opa || A_Sta_X || A_Sta_Y || A_End_X || A_End_Y || A_Lin_Opa)
			return true;
		else
			return false;
	}
...
public:
	...
	bool Init(POINT position, SIZE size, DCOLOR BackColor, DColorBox_Point* Points, UINT PointCount, POINT RadialCenter, float RadialRadiusX, float RadialRadiusY, float RoundRadiusX = 0.0f, float RoundRadiusY = 0.0f, bool CanMoveWindow = false)
	{
		if (Inited)
			return false;
...
private:
	...
	bool _IsContentChanging()
	{
		if (A_Col || A_RC_x || A_RC_y || A_RC_width || A_RC_height || A_Radius_X || A_Radius_Y || A_Opa || A_Cen_X || A_Cen_Y || A_RRad_X || A_RRad_Y || A_RRad_Opa)
			return true;
		else
			return false;
	}
