...
{
public:
	___AutoLock(___DThreadLock& lock)
	{
		l = lock;
		l.Lock();
	}
...
public:
	...
	inline void AddAni(float Start, float End, UINT nFrame, DBezier* BezierParam)
	{
		___CalcLock.Lock();
		if (!nFrame)
		{
...
public:
	...
	inline void AddAni(D2D1_COLOR_F Start, DCOLOR End, UINT nFrame)
	{
		r.AddAni(Start.r, (float)End.r, nFrame, nullptr);
		g.AddAni(Start.g, (float)End.g, nFrame, nullptr);
		b.AddAni(Start.b, (float)End.b, nFrame, nullptr);
		a.AddAni(Start.a, End.a, nFrame, nullptr);
		return;
	}
	inline bool FrameCalc(D2D1_COLOR_F& col)
	{
		bool res = false;
		if (r)
		{
...
public:
	...
	void SetTotalOpacity(float opa, UINT time = 0, DBezier* Nonlinear = nullptr, bool ReDraw = true)
	{
		if (time)
		{
			if (opa < 0.0f)
			{
				A_Opa.AddAni(this->TotalOpacity, 0.0f, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear);
				this->_SetNeedUpdate(true);
			}
			else if (opa > 1.0f)
			{
				A_Opa.AddAni(this->TotalOpacity, 1.0f, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear);
				this->_SetNeedUpdate(true);
			}
			else
			{
				A_Opa.AddAni(this->TotalOpacity, opa, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear);
				this->_SetNeedUpdate(true);
			}
			return;
		}
...
protected:
	...
	ID2D1Bitmap* LastBackImg = nullptr;
	...
	virtual bool m_SetState(DControlState state, bool Lock)
	{
		return false;
	}
	...
	inline D2D1_RECT_F _GetUpdateRect()
	{
		if (Parent)
		{
			...
			if (RectChanged)
			{
				...
				ParClip = CalcRectOverlap(CalcRectUnion(r, UpdateRC), ParClip);
			...
			ParClip = CalcRectOverlap(UpdateRC, ParClip);
			if (IsRectWorkable(ParClip))
				return ParClip;
			else return { 0 };
		}
			...
			if (RectChanged)
			{
				...
				RectChanged = false;
				___RcChangedLock.Unlock();
				return CalcRectUnion(r, UpdateRC);
			}
			___RcChangedLock.Unlock();
	...
	inline void _SetParentDrawing(bool drawing)
	{
		...
		if (ParentDrawing == true && !drawing && RectChanged)
			RELEASE(LastBackImg);
	...
	inline void _UpdateBackImg(ID2D1BitmapRenderTarget* Target)
	{
		...
		if (!BackImg)
		{
			...
			prop.dpiX = dpiX;
			prop.dpiY = dpiY;
			Target->CreateBitmap({ (UINT)((UpdateRC.right - UpdateRC.left) * dpiX / 96.0f), (UINT)((UpdateRC.bottom - UpdateRC.top) * dpiY / 96.0f) }, prop, &BackImg);
		}
		D2D1_POINT_2U dst = { 0, 0 };
		D2D1_RECT_U src = { (UINT)(UpdateRC.left * dpiX / 96.0f), (UINT)(UpdateRC.top * dpiY / 96.0f), (UINT)(UpdateRC.right * dpiX / 96.0f), (UINT)(UpdateRC.bottom * dpiY / 96.0f) };
	...
	inline void _EndDraw_CallChildToUpdateBackImg(ID2D1BitmapRenderTarget* CacheTarget)
	{
		...
		if (!Child.empty())
		{
			if (_IsColorChanging())//将Child全部设为要更新
			{
...
public:
	...
	void SetRadius(float RadiusX, float RadiusY, UINT time = 0, DBezier* Nonlinear = nullptr)
	{
		if (time)
		{
			A_Radius_X.AddAni(DstRC.radiusX, RadiusX, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear);
			A_Radius_Y.AddAni(DstRC.radiusY, RadiusY, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear);
			this->_SetNeedUpdate(true);
			return;
		}
	...
	void SetPosition(float x, float y, UINT time = 0, DBezier* Nonlinear = nullptr)
	{
		if (time)
		{
			A_RC_x.AddAni(DstRC.rect.left, x, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear);
			A_RC_y.AddAni(DstRC.rect.top, y, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear);
			this->_SetNeedUpdate(true);
			return;
		}
	...
	void SetSize(float width, float height, UINT time = 0, DBezier* Nonlinear = nullptr)
	{
		if (time)
		{
			A_RC_width.AddAni(DstRC.rect.right - DstRC.rect.left, width, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear);
			A_RC_height.AddAni(DstRC.rect.bottom - DstRC.rect.top, height, (UINT)(FrameRate * (time / 1000.0f)), Nonlinear);
			this->_SetNeedUpdate(true);
			return;
		}
		D2D1_RECT_F OldUpdateRC = UpdateRC;
...
private:
	...
	Ani_Color A_Col_Fill;
	Ani_Color A_Col_Stroke;
	Ani_Color A_Col_Text;
	...
	bool m_SetState(DControlState state, bool Lock)
	{
		if (Lock)
			___StateLock.Lock();
	...
	bool _SetNormalState(HWND hWnd, UINT Msg, UINT NowControlID = 0)
	{
		...
		return m_SetState(DControlState::Normal, false);
	}
	...
	bool _MouseMoveMsgProc(int xPos, int yPos)
	{
		...
		if (OnClick)
			return m_SetState(DControlState::Click, false);

		return m_SetState(DControlState::MouseMove, false);
	}
	...
	bool _Draw(ID2D1BitmapRenderTarget* DstTarget, D2D1_COLOR_F BackColor)
	{
		...
		if (Parent)//有底层控件, 要重画backimg
		{
			...
			{
				___RcChangedLock.Lock();
				if (RectChanged && LastBackImg)
				{
					DstTarget->DrawBitmap(LastBackImg, LastRect, 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR);
					RELEASE(LastBackImg);
					BackImg = nullptr;
					_UpdateBackImg(DstTarget);
				}
				else if (BackImg)
...
private:
	...
	bool _SetNormalState(HWND hWnd, UINT Msg, UINT NowControlID = 0)
	{
		...
		if (Child.find(NowControlID) != Child.end() && (Msg == WM_MOUSEMOVE || Msg == WM_LBUTTONDOWN || Msg == WM_LBUTTONUP))
		{
			if (Msg == WM_LBUTTONDOWN || Msg == WM_LBUTTONUP)
				m_SetState(DControlState::MouseMove, false);
			else
				HaveSetNormal = true;
		}
		else
	...
	bool _LMouseDownMsgProc(int xPos, int yPos)
	{
		...
		return m_SetState(DControlState::Edit, false);
	}
	bool _LMouseUpMsgProc(int xPos, int yPos)
	{
		___AutoLock lock(___StateLock);
	...
	bool ReDrawMem()
	{
		...
		if (MemDC)
		{
			...
			if (MemBmp)
			{
				...
				Rectangle(dc, EdgeOut + ShadowStrokeWidth / 2, EdgeOut + ShadowStrokeWidth / 2 + 2, leftWid + ShadowStrokeWidth * 2, rc.bottom - EdgeOut - ShadowStrokeWidth / 2 + 1);
				...
				MoveToEx(dc, 0, EdgeOut + ShadowStrokeWidth / 2, nullptr);
				LineTo(dc, topWid, EdgeOut + ShadowStrokeWidth / 2);
		...
		if (hWnd == this->hWnd && !Controls.empty())
		{
			...
			case WM_LBUTTONUP:
			{
				...
				if (con != Controls.end())
				{
					...
					if (onClick)
						proc(hWnd, conID, type, DControlMsg::Control_Click, lParam, 0);
				}
				else
				{
					___ConLock.Unlock();
...
inline void __cdecl DXUI_RenderThread(void* param)
{
	if (param)
	{
		...
		while (!ui->_GetExitRenderThreadFlag())
		{
			...
			if (Delay == 16 && ui->IsHardWareRendering())
			{
