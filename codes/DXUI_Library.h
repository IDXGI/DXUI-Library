...
enum class DControlMsg
{
	...
	TextBox_CharChange,
	TextBox_Scrolling,//wParam也是鼠标坐标
	TextBox_Enter,
};
...
public:
	...
	void SetRadius(float RadiusX, float RadiusY, UINT time = 0, DBezier* Nonlinear = nullptr)
	{
		...
		if (Parent)
			...
			this->_SetNeedUpdate(true, UpdateRC);
...
private:
	...
	bool _MouseMoveMsgProc(int xPos, int yPos, DControlMsg* msg)
	{
		...
		if (msg)
			*msg = DControlMsg::Control_MouseMove;
	...
	bool _LMouseDownMsgProc(int xPos, int yPos, DControlMsg* msg)
	{
		...
		if (msg)
			*msg = DControlMsg::Control_LButtonDown;
		OnClick = true;
	...
	bool _LMouseUpMsgProc(int xPos, int yPos, DControlMsg* msg)
	{
		...
		if (msg)
			*msg = DControlMsg::Control_LButtonUp;
		OnClick = false;
	...
	bool _Draw(ID2D1BitmapRenderTarget* DstTarget, D2D1_COLOR_F BackColor)
	{
		...
		f_FillColor.a = NowType.FillColor.a * TotalOpacity;
		//动画——描边颜色处理
		...
		f_StrokeColor.a = NowType.StrokeColor.a * TotalOpacity;
		//动画——文字颜色处理
		...
		f_TextColor.a = NowType.TextColor.a * TotalOpacity;
		...
		if (Parent)//有底层控件, 要重画backimg
		{
			...
			if (EnableClip)
			{
				DstTarget->FillRoundedRectangle({ UpdateClip, DstRC.radiusX, DstRC.radiusY }, brush);
			}
...
private:
	...
	bool _Draw(ID2D1BitmapRenderTarget* DstTarget, D2D1_COLOR_F BackColor)
	{
		...
		f_TextColor.a = NowType.a * TotalOpacity;
...
private:
	...
	bool _CharMsg(UINT Msg, WPARAM wParam, LPARAM lParam, DControlMsg* ConMsg)
	{
		...
		case WM_IME_CHAR:
		{
			...
			if (ConMsg)
				*ConMsg = DControlMsg::TextBox_CharChange;
			break;
		}
		...
		case WM_CHAR:
		{
			...
			case 0x0D://回车键
				if (ConMsg)
					*ConMsg = DControlMsg::TextBox_Enter;
	...
	bool _SetNormalState(HWND hWnd, UINT Msg, UINT NowControlID = 0)
	{
		...
		if (State == DControlState::Edit && (Msg == WM_MOUSEMOVE || Msg == WM_NCMOUSEMOVE || Msg == WM_LBUTTONUP || Msg == WM_NCLBUTTONUP || Msg == WM_MOUSELEAVE))
			return false;
	...
	{
		if (msg)
			*msg = DControlMsg::TextBox_Scrolling;
		...
		if (hWnd == this->hWnd && !Controls.empty())
		{
			...
			case WM_MOUSEMOVE:
			{
				...
				if (con != Controls.end())
				{
					...
					{
						//通知给用户
						proc(hWnd, conID, type, DControlMsg::Control_StateChanged, before, now);
					}
					//通知给用户
					proc(hWnd, conID, type, Msg, lParam, 0);
				}
				else {
					___ConLock.Unlock();
			...
			case WM_LBUTTONDOWN:
			{
				...
				if (con != Controls.end())
				{
					...
					if (con->second->_LMouseDownMsgProc(xPos, yPos, &Msg))
					{
						now = (LPARAM)con->second->State;
					}
			...
			case WM_LBUTTONUP:
			{
				...
				if (con != Controls.end())
				{
					...
					if (con->second->_LMouseUpMsgProc(xPos, yPos, &Msg))
					{
						now = (LPARAM)con->second->State;
					}
					___ConLock.Unlock();
			...
			case WM_IME_CHAR:
			{
				...
				if (!ActiveCon.empty())
				{
					...
					while (con != ActiveCon.end())
					{
						if (con->second->_CharMsg(message, wParam, lParam, &Msg))
						{
							ReturnValue = 0;
			...
			case WM_MOUSEWHEEL:
			{
				...
				if (con != Controls.end())
				{
					if (con->second->_MouseWheelMsgProc(&Msg, wParam))
					{
						ReturnValue = 1;
			...
			case WM_SIZE:
			{
				...
				Msg = DControlMsg::Window_Size;
				ReturnValue = 0;

				//通知给用户
				proc(hWnd, 0, DControlType::Null, Msg, 0, 0);
				break;
			}
			case WM_SHOWWINDOW:
			{
...
private:
	...
	{
		std::map<DControl*, WPARAM> NormalCon;
		...
		if (!ActiveCon.empty())
		{
			...
			while (con != ActiveCon.end())
			{
				before = (WPARAM)con->second->State;//将更改之前的状态暂时保存起来
				...
				if (con->second->_SetNormalState(hWnd, Msg, NowControlID))
				{
					...
					NormalCon.insert(std::pair<DControl*, WPARAM>(con->second, before));
		...
		if (!NormalCon.empty())
		{
			std::map<DControl*, WPARAM>::iterator con = NormalCon.begin();
			while (con != NormalCon.end())
			{
				//通知给用户
				proc(hWnd, con->first->ID, con->first->ControlType, DControlMsg::Control_StateChanged, con->second, (LPARAM)DControlState::Normal);
