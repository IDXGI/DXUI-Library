...
public:
	...
	bool Init(POINT position, SIZE size, DTextBoxTypeStyle Normal, DTextBoxTypeStyle MouseMove, DTextBoxTypeStyle Edit, DTextBoxTypeStyle Disable, float radiusX, float radiusY, float StrokeWidth, std::wstring FamliyName, float FontSize, bool AutoWarp = true, bool MultiLine = true, bool ReadOnly = false, DFontWeight FontWeight = NormalWeight, DAlignment X_Alignment = Near, DAlignment Y_Alignment = Near, DCOLOR CaretColor = { 0, 0, 0, 1.0f }, float CaretWidth = 1.5f, DCOLOR SelectFillColor = { 0, 0, 0, 0.45f })
	{
		DstRC.rect.left = (float)position.x;
		DstRC.rect.top = (float)position.y;
		DstRC.rect.right = (float)position.x + size.cx;
...
private:
	...
	bool _Draw(ID2D1BitmapRenderTarget* DstTarget, D2D1_COLOR_F BackColor, D2D1_RECT_F MainWndRC)
	{
		...
		//画插入符
		if (Insert_Visible && State == TB_Edit && CaretColor.a != 0.0f)
		{
