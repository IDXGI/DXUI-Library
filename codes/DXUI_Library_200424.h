...
/******************************************************************
	...
	DXUI 所有函数统一使用 <设备独立像素(DIP)> ，内部会自动根据DPI来缩放
	由于 DXUI 支持高DPI环境，请在编译时设置 <高DPI识别(DPI Aware)>，防止系统缩放模糊
...
#define TextBox_SmoothScrollTime 150//文本框的平滑滚动时间(ms)
#define TextBox_CaretMoveTime 100//文本框的插入符平滑移动时间(ms)
#define TextBox_SmoothScrollBezier { 0.33f, 1.0f, 0.68f, 1.0f }//文本框的平滑滚动贝塞尔曲线参数
...
enum class DControlMsg
{
	...
	TextBox_TextChange,
	TextBox_Scrolling,//鼠标滚动消息触发时wParam是鼠标坐标,lParam是Delta，当滚动动画结束时也会发送一次，但此时两个参数都是0
	TextBox_Enter,
...
public:
	...
	inline void AddAni(float Start, float End, UINT nFrame, DBezier* BezierParam, UINT AID)
	{
		___CalcLock.Lock();
		if (!nFrame)
		{
	...
	inline float FrameCalc(UINT& aid)
	{
		___CalcLock.Lock();
...
public:
	...
	inline D2D1_RECT_F _GetClipRC()
	{
		if (Parent)
		{
			...
			return { std::fmaxf(this->ClipRC.left, rc.left), std::fmaxf(this->ClipRC.top, rc.top), std::fminf(this->ClipRC.right, rc.right), std::fminf(this->ClipRC.bottom, rc.bottom) };
		}
		else
			return this->ClipRC;
	}
...
protected:
	...
	{
		DstTarget->PushAxisAlignedClip(rect, D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);
		DstTarget->Clear(col);
	...
	inline void _NoParentClearBackGru(ID2D1RenderTarget* DstTarget, const D2D1_COLOR_F& BackColor)
	{
	...
	void _PreDisposeRC()
	{
		...
		if (StrokeWidth != 0.0f)
		{
			UpdateRC.left = ((int)((DstRC.rect.left - StrokeWidth / 2.0f) * (float)dpiX / 96.0f) - 1) * 96.0f / (float)dpiX;
			UpdateRC.top = ((int)((DstRC.rect.top - StrokeWidth / 2.0f) * (float)dpiY / 96.0f) - 1) * 96.0f / (float)dpiY;
			if (UpdateRC.left < 0.0f)
				UpdateRC.left = 0.0f;
			if (UpdateRC.top < 0.0f)
				UpdateRC.top = 0.0f;
			UpdateRC.right = ((int)((DstRC.rect.right + StrokeWidth / 2.0f) * (float)dpiX / 96.0f) + 2) * 96.0f / (float)dpiX;
			UpdateRC.bottom = ((int)((DstRC.rect.bottom + StrokeWidth / 2.0f) * (float)dpiY / 96.0f) + 2) * 96.0f / (float)dpiY;
		}
		else {
			UpdateRC.left = (int)(DstRC.rect.left * (float)dpiX / 96.0f) * 96.0f / (float)dpiX;
			UpdateRC.top = (int)(DstRC.rect.top * (float)dpiY / 96.0f) * 96.0f / (float)dpiY;
			UpdateRC.right = ((int)(DstRC.rect.right * (float)dpiX / 96.0f) + 1) * 96.0f / (float)dpiX;
			UpdateRC.bottom = ((int)(DstRC.rect.bottom * (float)dpiY / 96.0f) + 1) * 96.0f / (float)dpiY;
		}
		return;
	}
	...
	void _UpdateBackImg(ID2D1RenderTarget* Target, D2D1_RECT_F* UpdateClip = nullptr)
	{
		...
		{
			___UpdateClipLock.Lock();
			D2D1_RECT_F r = CalcRectOverlap(CalcRectOverlap(*UpdateClip, UpdateRC), WndRc);
			___UpdateClipLock.Unlock();
			if (!IsRectWorkable(r))
				return;

			src.left = (UINT)(r.left * (float)dpiX / 96.0f);
			src.top = (UINT)(r.top * (float)dpiY / 96.0f);
			src.right = (UINT)(r.right * (float)dpiX / 96.0f);
			src.bottom = (UINT)(r.bottom * (float)dpiY / 96.0f);
			dst = { (UINT)((r.left - UpdateRC.left) * dpiX / 96.0f), (UINT)((r.top - UpdateRC.top) * dpiY / 96.0f) };
		...
		else
		{
			...
			if (UpdateRC.left < 0.0f)
				dst.x = (UINT)(std::fabsf(UpdateRC.left) * (float)dpiX / 96.0f);
			else
				dst.x = 0;

			if (UpdateRC.top < 0.0f)
				dst.y = (UINT)(std::fabsf(UpdateRC.top) * (float)dpiY / 96.0f);
			else
				dst.y = 0;

			src.left = (UINT)(max(UpdateRC.left, 0.0f) * (float)dpiX / 96.0f);
			src.top = (UINT)(max(UpdateRC.top, 0.0f) * (float)dpiY / 96.0f);
	...
	virtual bool _Draw(ID2D1RenderTarget* DstTarget, const D2D1_COLOR_F& BackColor) = 0;

	void _EndDraw_CallChildToUpdateBackImg(ID2D1RenderTarget* CacheTarget)
	{
		___ChildLock.Lock();
		if (!Child.empty())
		{
			if (_IsContentChanging())//将Child全部设为要更新
			{
...
private:
	...
	{
		if (A_Col_Fill || A_Col_Stroke || A_Col_Text || A_Radius_X || A_Radius_Y || A_Opa)
			return true;
		else
			return false;
	}
	...
	bool m_SetState(DControlState state, bool NeedUnlock = false)
	{
		if (State != state)
		{
			...
			case DControlState::Normal:
				//填充颜色
				A_Col_Fill.AddAni(NowType.FillColor, NormalType.FillColor, (UINT)(FrameRate * time));
				//描边颜色
				A_Col_Stroke.AddAni(NowType.StrokeColor, NormalType.StrokeColor, (UINT)(FrameRate * time));
				//文字颜色
				A_Col_Text.AddAni(NowType.TextColor, NormalType.TextColor, (UINT)(FrameRate * time));
				break;
			case DControlState::MouseMove:
				//填充颜色
				A_Col_Fill.AddAni(NowType.FillColor, MouseMoveType.FillColor, (UINT)(FrameRate * time));
				//描边颜色
				A_Col_Stroke.AddAni(NowType.StrokeColor, MouseMoveType.StrokeColor, (UINT)(FrameRate * time));
				//文字颜色
				A_Col_Text.AddAni(NowType.TextColor, MouseMoveType.TextColor, (UINT)(FrameRate * time));
				break;
			case DControlState::Click:
				//填充颜色
				A_Col_Fill.AddAni(NowType.FillColor, ClickType.FillColor, (UINT)(FrameRate * time));
				//描边颜色
				A_Col_Stroke.AddAni(NowType.StrokeColor, ClickType.StrokeColor, (UINT)(FrameRate * time));
				//文字颜色
				A_Col_Text.AddAni(NowType.TextColor, ClickType.TextColor, (UINT)(FrameRate * time));
				break;
			case DControlState::Disable:
				//填充颜色
				A_Col_Fill.AddAni(NowType.FillColor, DisableType.FillColor, (UINT)(FrameRate * time));
				//描边颜色
				A_Col_Stroke.AddAni(NowType.StrokeColor, DisableType.StrokeColor, (UINT)(FrameRate * time));
				//文字颜色
				A_Col_Text.AddAni(NowType.TextColor, DisableType.TextColor, (UINT)(FrameRate * time));
				break;
			}
	...
	bool _Draw(ID2D1RenderTarget* DstTarget, const D2D1_COLOR_F& BackColor)
	{
		if (!DstTarget)
		{
			this->_SetNeedUpdate(true);
...
public:
	...
	inline bool Init(POINT position, SIZE size, DCOLOR color, bool CanMoveWindow, std::wstring text, std::wstring FamliyName, float FontSize, bool AutoWarp = true, DFontWeight FontWeight = DFontWeight::NormalWeight, DAlignment X_Alignment = DAlignment::Near, DAlignment Y_Alignment = DAlignment::Near, DMouseStyle MouseStyle = DMouseStyle::BasicMouse)
	{
		return Init(position, size, { color, false, false, 0 }, { color, false, false, 0 }, { color, false, false, 0 }, CanMoveWindow, text, FamliyName, FontSize, AutoWarp, FontWeight, X_Alignment, Y_Alignment, MouseStyle);
	}
...
private:
	...
	bool _IsContentChanging()
	{
		if (A_Col || A_Radius_X || A_Radius_Y || A_Opa)
			return true;
		else
			return false;
	}
	...
	bool m_SetState(DControlState state, bool NeedUnlock = false)
	{
		if (State != state && state != DControlState::Disable)
		{
			...
			case DControlState::Normal:
				//文字颜色
				A_Col.AddAni(NowType, NormalType.TextColor, (UINT)(FrameRate * time));
				break;
			case DControlState::MouseMove:
				//文字颜色
				A_Col.AddAni(NowType, MouseMoveType.TextColor, (UINT)(FrameRate * time));
				break;
			case DControlState::Click:
				//文字颜色
				A_Col.AddAni(NowType, ClickType.TextColor, (UINT)(FrameRate * time));
				break;
			}
...
public:
	...
	void AddChar(const wchar_t Ch, bool MoveCaret = true)
	{
		...
		//立即刷新插入符
		if (MoveCaret)
			this->MoveCaret(Text.size(), SmoothScr, false);
	...
	void InsertChar(UINT TextPos, const wchar_t Ch, bool MoveCaret = true)
	{
		...
		//立即刷新插入符
		if (MoveCaret)
			this->MoveCaret_Offset(1, SmoothScr, false);
	...
	void InsertText(UINT TextPos, std::wstring str, bool MoveCaret = true)
	{
		...
		//立即刷新插入符
		if (MoveCaret)
			this->MoveCaret(textpos + str.size(), SmoothScr, false);
	...
	void EraseText(UINT TextPos, UINT Count, bool MoveCaret = true)
	{
		...
		//立即刷新插入符
		if (MoveCaret)
			this->MoveCaret(textpos, SmoothScr, false);
	...
	void EraseText(UINT Count, bool MoveCaret = true)
	{
		...
		//立即刷新插入符
		if (MoveCaret)
			this->MoveCaret_Offset(0, SmoothScr, false);
	...
	void MoveCaret_Offset(long OffsetPos, bool SmoothScr = true, bool ReDraw = true)
	{
		...
		long pos = (long)(Insert_TextPos + OffsetPos);
...
private:
	...
	bool _IsContentChanging()
	{
		if (A_Col_Fill || A_Col_Stroke || A_Col_Text || A_ScrX || A_ScrY || A_InsX || A_InsY || A_Radius_X || A_Radius_Y || A_Opa)
			return true;
		else
			return false;
	}
	...
	bool m_SetState(DControlState state, bool NeedUnlock = false)
	{
		if (State != state)
		{
			...
			case DControlState::Edit:
				//填充颜色
				A_Col_Fill.AddAni(NowType.FillColor, EditType.FillColor, (UINT)(FrameRate * time));
				//描边颜色
				A_Col_Stroke.AddAni(NowType.StrokeColor, EditType.StrokeColor, (UINT)(FrameRate * time));
				//文字颜色
				A_Col_Text.AddAni(NowType.TextColor, EditType.TextColor, (UINT)(FrameRate * time));
				break;
			case DControlState::Disable:
				//填充颜色
				A_Col_Fill.AddAni(NowType.FillColor, DisableType.FillColor, (UINT)(FrameRate * time));
	...
	inline void ExitComp(HWND hWnd, bool ReDraw)
	{
		...
		{
			//设置输入字符串为空
			ImmSetCompositionStringW(hIMC, SCS_SETSTR, nullptr, 0, (void*)L"", sizeof(WCHAR));
			ImmSetCompositionStringW(hIMC, SCS_SETRECONVERTSTRING, nullptr, 0, (void*)L"", sizeof(WCHAR));
	...
	bool _KeyDownMsg(WPARAM wParam)
	{
		if (State == DControlState::Edit)
		{
			...
			case VK_DOWN:
			{
				...
				this->ResetInsertPos((int)(Insert_PixelPos.left - Scr_Offset.x * (float)dpiX / 96.0f), (int)(Insert_PixelPos.bottom - (Scr_Offset.y - 2.0f) * (float)dpiY / 96.0f));
				break;
			}
			case 'V':
			{
		...
		case WM_CHAR:
		{
			...
			case 0x08://退格键
				...
				if (Text.size())
				{
					...
					{
						___TextLock.Unlock();
						this->EraseText(Select_BeginPos, Select_EndPos - Select_BeginPos, true);
						this->ExitSelectMode(false);
					}
					else
					{
	...
	bool _MouseWheelMsgProc(HWND hWnd, WPARAM wParam, LPARAM lParam)
	{
		...
		if (Delta > 0)//手指向前滑动时
		{
			if (ScrY > 0)
			{
				...
				if (ScrY != Scr_Offset.y)
				{
					if (SmoothScr)
					{
						DBezier be = TextBox_SmoothScrollBezier;
						A_ScrY.AddAni(src, ScrY, (UINT)(FrameRate * (TextBox_SmoothScrollTime / 1000.0f)), &be, 0);
					}
					else
					{
						Scr_Offset.y = ScrY;
	...
	void _DisposeAnimation()//由主窗口每帧后主动调用此函数，来实现每次的动画
	{
		...
		if (A_ScrX)
		{
			...
			{
				//通知给用户
				mSendConMsg(DControlMsg::TextBox_Scrolling, 0, 0);
			}
		}
		if (A_ScrY)
		{
	...
	bool _Draw(ID2D1RenderTarget* DstTarget, const D2D1_COLOR_F& BackColor)
	{
		...
		//画插入符
		if (Insert_Visible && !SelectMode && State == DControlState::Edit && CaretColor.a != 0.0f)
		{
	...
	void ResetTextScrolling(bool ScrAni = false, bool Redraw = true)
	{
		...
		if (srcX != ScrX || (srcY != ScrY && MultiLine))//只有是多行时，才应用垂直滚动
		{
			...
			{
				DBezier be = TextBox_SmoothScrollBezier;
				A_ScrX.AddAni(srcX, ScrX, (UINT)(FrameRate * (TextBox_SmoothScrollTime / 1000.0f)), &be, 0);
				if (MultiLine)//水平滚动进来后，避免掉非多行时的设置
					A_ScrY.AddAni(srcY, ScrY, (UINT)(FrameRate * (TextBox_SmoothScrollTime / 1000.0f)), &be, 0);
	...
	void ResetInsertPos(int xPos, int yPos, bool InsAnimation = true)
	{
		...
		if (IsBehind)
		{
			...
			{
				if (A_InsX)
					A_InsX.AddAni(A_InsX.GetEnd(), TextOutputRC.left + hit.left + hit.width, (UINT)(FrameRate * (TextBox_CaretMoveTime / 1000.0f)), &be, 0);
				else
		...
		else
		{
			...
			{
				if (A_InsX)
					A_InsX.AddAni(A_InsX.GetEnd(), TextOutputRC.left + hit.left, (UINT)(FrameRate * (TextBox_CaretMoveTime / 1000.0f)), &be, 0);
				else
		...
		{
			if (A_InsY)
				A_InsY.AddAni(A_InsY.GetEnd(), TextOutputRC.top + hit.top, (UINT)(FrameRate * (TextBox_CaretMoveTime / 1000.0f)), &be, 0);
			else
	...
	inline bool RecreateTextInterface(bool ResetScr = true)//注意，此函数不上锁，要放在锁定区域内调用
	{
		...
		{
			___TextLock.Lock();
			DWriteFactory->CreateTextLayout((Text.substr(0, Insert_TextPos) + Comp_Str + Text.substr(Insert_TextPos, Text.size() - Insert_TextPos)).c_str(), Text.size() + Comp_Str.size(), TextFormat, TextOutputRC.right - TextOutputRC.left, TextOutputRC.bottom - TextOutputRC.top, &TextLayout);
			___TextLock.Unlock();

			if (TextLayout)
			{
				if (Comp_Str.size())
					TextLayout->SetUnderline(true, { Insert_TextPos, Comp_Str.size() });
...
public:
	...
	bool GaussianBlur(float Sigma, UINT Threads = 4)
	{
		...
		___BmpLock.Lock();
		_Gau_Blur(CacheData, (UINT)((BmpRC.right - BmpRC.left) * (float)dpiX / 96.0f), (UINT)((BmpRC.bottom - BmpRC.top) * (float)dpiY / 96.0f), Pitch, Sigma, Threads);
...
private:
	...
	bool _IsContentChanging()
	{
		if (A_Fill || A_Ang || A_Radius_X || A_Radius_Y || A_Opa)
			return true;
		else
			return false;
	}
	...
	inline bool MyLoadBmp(ID2D1RenderTarget* Target, BYTE* data, ULONG Size, ID2D1Bitmap** bmp, D2D1_RECT_F* bmprc)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				{
					RECT GoodRc = { 0 };
					GoodRc = RectSizeWithConstantRatio({ 0, 0, (long)(DstSize.width), (long)(DstSize.height) }, { (long)wid, (long)hei });
					...
					DstSize.width = DstSize.width;
					DstSize.height = DstSize.height;
...
public:
	...
	bool Init(POINT position, SIZE size, DCOLOR color, float radiusX = 0.0f, float radiusY = 0.0f, bool CanMoveWindow = false, DMouseStyle MouseStyle = DMouseStyle::BasicMouse)
	{
		if (Inited)
			return false;
		...
		this->MouseStyle = MouseStyle;
		this->ControlType = DControlType::DSolidColorBox;
	...
	void Release()
	{
		...
		ControlType = DControlType::DSolidColorBox;
		MouseStyle = DMouseStyle::BasicMouse;
	...
	inline void SetColor(DCOLOR col, UINT time = 0)
	{
		DCOLOR c = col;
		DetectAndResetColor(c);
		if (time)
		{
			A_Col.AddAni(mColor, c, (UINT)(FrameRate * (time / 1000.0f)));
			this->_SetNeedUpdate(true);
			return;
		}
...
protected:
	...
	D2D1_COLOR_F mColor = { 0 };//存储以255为最大的颜色
	_Ani_Color A_Col;
	...
	void _DrawHitTestBmp()//绘制命中测试BMP
	{
		...
		if (bru)
		{
			D2D1_ROUNDED_RECT hittest = { {1.0f, 1.0f, DstSize.width, DstSize.height}, DstRC.radiusX, DstRC.radiusY };
			HitTestBmpTarget->BeginDraw();
			HitTestBmpTarget->Clear(D2D1::ColorF(D2D1::ColorF::Black));
	...
	bool _Draw(ID2D1RenderTarget* DstTarget, const D2D1_COLOR_F& BackColor)
	{
		...
		//动画——填充颜色
		f_FillColor = Calc_fColor(mColor, TotalOpacity);
...
public:
	...
	bool Init(POINT position, SIZE size, DCOLOR BackColor, DColorBox_Point* Points, UINT PointCount, POINT StartPos, POINT EndPos, float radiusX = 0.0f, float radiusY = 0.0f, bool CanMoveWindow = false, DMouseStyle MouseStyle = DMouseStyle::BasicMouse)
	{
		if (Inited)
			return false;
		...
		this->MouseStyle = MouseStyle;
		this->ControlType = DControlType::DLinearColorBox;
	...
	void Release()
	{
		...
		ControlType = DControlType::DLinearColorBox;
		MouseStyle = DMouseStyle::BasicMouse;
...
private:
	...
	bool _IsContentChanging()
	{
		if (A_Col || A_Radius_X || A_Radius_Y || A_Opa || A_Sta_X || A_Sta_Y || A_End_X || A_End_Y || A_Lin_Opa)
			return true;
		else
			return false;
	}
...
public:
	...
	bool Init(POINT position, SIZE size, DCOLOR BackColor, DColorBox_Point* Points, UINT PointCount, POINT RadialCenter, float RadialRadiusX, float RadialRadiusY, float RoundRadiusX = 0.0f, float RoundRadiusY = 0.0f, bool CanMoveWindow = false, DMouseStyle MouseStyle = DMouseStyle::BasicMouse)
	{
		if (Inited)
			return false;
		...
		this->MouseStyle = MouseStyle;
		this->ControlType = DControlType::DRadialColorBox;
	...
	void Release()
	{
		...
		ControlType = DControlType::DRadialColorBox;
		MouseStyle = DMouseStyle::BasicMouse;
...
private:
	...
	bool _IsContentChanging()
	{
		if (A_Col || A_Radius_X || A_Radius_Y || A_Opa || A_Cen_X || A_Cen_Y || A_RRad_X || A_RRad_Y || A_RRad_Opa)
			return true;
		else
			return false;
	}
	...
	inline bool _RenderFrame(bool Lock = true)
	{
		...
		if (!Controls.empty())
		{
			...
			for (size_t i = 0; i < AllCount; ++i, ++con)
			{
				...
				if (con->second && con->second->_GetNeedUpdate() && !con->second->Parent)//这里顺序不能错
				{
					if (!Begined)
					{
						RenderTarget->BeginDraw();
					...
					con->second->_NoParentClearBackGru(RenderTarget, BackColor);
				}//NeedUpdate End
			...
			for (size_t i = 0; i < AllCount; ++i, ++con)
			{
				if (con->second && con->second->NeedUpdate)
				{
					if (!Begined)
					{
						RenderTarget->BeginDraw();
					...
					con->second->_Draw(RenderTarget, BackColor);//将当前控件画到缓存渲染目标上
					...
					if (Dbg_ShowUpdate)
					{
						...
						if (b)
						{
							RenderTarget->DrawRectangle(con->second->_GetUpdateRect(), b, 1.0f);
							b->Release();
						}
					}
	...
	LRESULT DXUIMessageProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		if (hWnd == this->hWnd && !Controls.empty())
		{
			...
			case WM_MOUSEWHEEL:
			{
				...
				if (con != Controls.end())
				{
					if (con->second->_MouseWheelMsgProc(hWnd, wParam, lParam))
					{
						ReturnValue = 1;
					}
				}
		...
		if (!Controls.empty())
		{
			...
			for (size_t i = 0; i < AllCount; ++i, ++con)
			{
				...
				if (con->second && con->second->_GetNeedUpdate() && !con->second->Parent)//这里顺序不能错
				{
					...
					con->second->_NoParentClearBackGru(DCTarget, BackColor);
				}//NeedUpdate End
			...
			for (size_t i = 0; i < AllCount; ++i, ++con)
			{
				if (con->second && con->second->NeedUpdate)
				{
					...
					con->second->_Draw(DCTarget, BackColor);//将当前控件画到渲染目标上
					...
					if (Dbg_ShowUpdate)
					{
						...
						if (b)
						{
							DCTarget->DrawRectangle(con->second->_GetUpdateRect(), b, 1.0f);
							b->Release();
						}
					}
